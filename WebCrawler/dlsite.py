...
def getStudio(a):
    html = etree.fromstring(a, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    try:
        result = html.xpath('//th[contains(text(),"サークル名")]/../td/span[1]/a/text()')[0]
    except:
        result = ''
    return result
def getRuntime(a):
...
def getLabel(a):
    ...
    try:
        try:
            result = html.xpath('//th[contains(text(),"シリーズ名")]/../td/span[1]/a/text()')[0]
        except:
            result = html.xpath('//th[contains(text(),"サークル名")]/../td/span[1]/a/text()')[0]
    except:
        result = ''
...
def getSeries(a):
    html = etree.fromstring(a, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    try:
        result = html.xpath('//th[contains(text(),"シリーズ名")]/../td/a/text()')[0]
    except:
        result = ''
    return result
...
def get_html(url):
    ...
    result = requests.get(str(url), headers=headers, cookies=cookies, proxies=proxies)
